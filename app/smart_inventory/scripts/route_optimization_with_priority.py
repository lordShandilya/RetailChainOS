...
import logging
import numpy as np
from datetime import datetimeeorder_alerts to prioritize urgent deliveries in get_delivery_demands.
...
def get_delivery_demands():
    try:
        ...
        if df.empty:
            ...
            return {}
        logging.debug(f"Demands DataFrame (raw): {df[['sku_id', 'store_location', 'demand', 'priority_score']].to_dict()}")
        ...
        logging.debug(f"Average priorities: {priorities}")
            ...
            for vehicle_id in range(data['num_vehicles']):
                ...
                while not routing.IsEnd(index):
                    ...
                    if node != 0:
                        ...
                        estimated_time = int(max(1, (prev_distance / 60) + 0.25))
